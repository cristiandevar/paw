<div id="ejemploCarousel" class="carousel slide" data-ride="carousel">
    ...
    <li data-target="#ejemploCarousel" data-slide-to="{{ $loop->index }}" class="{{ $loop->first ? 'active' : '' }}" aria-current="true" aria-label="Slide {{$loop->index + 1}}">
    </button>
    @endforeach
    </ol>
    ...
    <a href="{{ url("your url!!!") }}">
    <img style="height:500px;" class="d-block w-100" src="{{ url($item->url) }}"
    alt="{{ $item->title }}">
    </a>
